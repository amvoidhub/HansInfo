...
for i, item in enumerate(ccl[0]):
    ...
    lx = re.findall('<span class="cino">\\(1\\)</span> \\((.*?)。', response.text)
    lx.append('无')
    if item in rtl.keys():
        rt = rtl[item]
    else:
    ...
    if item in zpl.keys():
        ...
        zp = 0
    data = [item, py[0:int(len(py)/4)], bh[0], jg[3], lx[0], '字表一', rt, zp]
...
for i, item in enumerate(ccl[1]):
    ...
    if item in zpl.keys():
        ...
        zp = 0
    data = [item, py[0:int(len(py)/4)], bh[0], jg[3], lx[0], '字表二', rt, zp]
    for j in range(len(data)):
...
for i, item in enumerate(hans):
    ...
    if item in zpl.keys():
        ...
        zp = 0
    data = [item, py[0:int(len(py)/4)], bh[0], jg[3], lx[0], zb, rt, zp]
    for j in range(len(data)):
